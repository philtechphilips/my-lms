...
@section('title')
    {{getenv('APP_FULL_NAME')}} | Administrator | Upload Course Demo Video
@endsection
...
<script>
    ...
    _('drag_drop').ondrop = function(event)
    {
        ...
        else
        {
            ...
            ajax_request.open("post", "/administrator/upload-video-db/{{Crypt::encrypt($id)}}");
    ...
    $(document).ready(function() {
        $('#cert_input').on('change', function() {
            ...
            $('#cert_form').submit(function(event) {
                ...
                $.ajax({
                    url: '/administrator/upload-video-db-input/{{Crypt::encrypt($id)}}',
                    method: 'POST',
                    data: formData,
                    dataType: 'JSON',
                    contentType: false,
