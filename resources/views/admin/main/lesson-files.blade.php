...
@section('title')
    {{getenv('APP_FULL_NAME')}} | Administrator | Upload Lesson Files
@endsection
            ...
            <div class="col-md-12">
                <div class="card d-flex justify-content">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Upload Lesson File</h4>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    _('drag_drop').ondrop = function(event)
    {
        ...
        {
            if(!['image/jpeg', 'image/png', 'video/mp4', 'audio/mp3', 'application/pdf'].includes(drop_files[count].type))
            {
                error += '<div class="alert alert-danger"><b>'+image_number+'</b> Selected File must be .jpg / .png / .mp4 / mp3 Only.</div>';
            }
            else
            {
        ...
        else
        {
            ...
            ajax_request.open("post", "/administrator/upload-lesson-file/{{Crypt::encrypt($id)}}");
    ...
    $(document).ready(function() {
        $('#cert_input').on('change', function() {
            ...
            $('#cert_form').submit(function(event) {
                ...
                $.ajax({
                    url: '/administrator/upload-lesson-files-input/{{Crypt::encrypt($id)}}',
                    method: 'POST',
                    data: formData,
                    dataType: 'JSON',
                    contentType: false,
