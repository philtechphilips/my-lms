...
@section('title')
{{getenv('APP_FULL_NAME')}} | Administrator | Upload Lesson Video
@endsection
...
<script>
    ...
    _('drag_drop').ondrop = function(event)
    {
        ...
        else
        {
            ...
            ajax_request.open("post", "/administrator/upload-lesson-video-db/{{Crypt::encrypt($id)}}");
    ...
    $(document).ready(function() {
        $('#cert_input').on('change', function() {
            ...
            $('#cert_form').submit(function(event) {
                ...
                $.ajax({
                    url: '/administrator/upload-lesson-video-db-input/{{Crypt::encrypt($id)}}',
                    method: 'POST',
                    data: formData,
                    dataType: 'JSON',
                    contentType: false,
